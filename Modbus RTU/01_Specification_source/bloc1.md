...
Ce champ représente l’état global du capteur du point de vue exploitation.
...
Exemple
Un capteur avec carte SD absente peut être en état dégradé.
...
Exemple
Un capteur configuré en mode événementiel peut rester en attente pendant une longue période.
...
Exemple
Une carte SD pleine empêche l’enregistrement de nouvelles campagnes.
...
Exemple
Une horloge non synchronisée rend les données difficilement exploitables en analyse croisée.
...
Plage recommandée
1. -20°C à +70°C (à ajuster selon matériel)

Valeurs particulières
1. valeur hors plage = condition anormale
2. valeur extrême = capteur potentiellement défaillant

Exemple
Un capteur installé près d’un moteur peut atteindre des températures élevées.

## Tension alimentation

Utilisation métier
1. détection de chute de tension
2. diagnostic alimentation instable
3. prévention de perte de données

Plage recommandée
à définir selon architecture (ex : 3.3V, 5V, 12V)

Exemple
Une tension instable peut provoquer des redémarrages intempestifs.

## Compteur d’erreurs

Utilisation métier
1. suivi de la fiabilité du capteur
2. détection de dégradation progressive
3. analyse post-mission

Exemple
Un compteur d’erreurs en augmentation indique un problème latent.

## Flags défaut (bitfield)

Utilisation métier
1. diagnostic rapide sans lecture détaillée
2. identification immédiate des défauts critiques

Exemples de défauts
1. carte SD absente
2. erreur capteur MEMS
3. erreur écriture
4. température hors plage
5. alimentation instable

Contrainte
Les bits non reconnus doivent être ignorés par la centrale.

## Compteur de redémarrages

Utilisation métier
1. détection de redémarrages anormaux
2. diagnostic d’instabilité système

Exemple
Un nombre élevé de redémarrages indique un problème électrique ou logiciel.
