...
function render(payload){
 ...
 const b1=t.systemState,b2=t.timeState,b3=t.vibrationState,b4=t.configurationState,b7=t.diagnosticState;
 ...
 dl('summaryFreshness',[["B1",freshness(b1)],["B2",freshness(b2)],["B3",freshness(b3)],["B4",freshness(b4)],["B7",freshness(b7)]]);
 dl('summaryState',[["Session",d.sessionState],["État B1",b1?.hasValue?systemStatus(b1.value.systemStatus):'—'],["Acquisition",b1?.hasValue?acquisition(b1.value.acquisitionState):'—'],["Config active",b4?.hasValue?fmt(b4.value.activeConfigId):'—'],["Santé B7",b7?.hasValue?health(b7.value.systemHealthStatus):'—']]);

 if(b3?.hasValue){const v=b3.value;dl('vibrationGlobal',[["RMS global",fmt(v.rmsGlobalMg,' mg')],["Crête globale",fmt(v.peakGlobalMg,' mg')],["Statut",b3Status(v.statusGlobal)],["Sévérité",severity(v.severityGlobal)]]);dl('vibrationAxes',[["RMS X",fmt(v.rmsXMg,' mg')],["RMS Y",fmt(v.rmsYMg,' mg')],["RMS Z",fmt(v.rmsZMg,' mg')],["Crête X",fmt(v.peakXMg,' mg')],["Crête Y",fmt(v.peakYMg,' mg')],["Crête Z",fmt(v.peakZMg,' mg')],["Axe dominant",axis(v.dominantAxis)]]);dl('vibrationQuality',[["Fraîcheur PC",`${freshness(b3)} · ${availability(b3)}`],["Validity flags",hex(v.validityFlags)],["Alarm flags",hex(v.alarmFlags)],["Âge valeur TR2",fmt(v.valueAgeMilliseconds,' ms')],["Dernière MAJ TR2",dateSeconds(v.lastUpdateTr2Seconds)],["Fenêtre",fmt(v.windowDurationMilliseconds,' ms')],["Échantillons valides",fmt(v.validSampleCount)]]);dl('vibrationCounters',[["Séquence calcul",fmt(v.calculationSequence)],["Dépassements",fmt(v.exceedCount)],["Alarmes",fmt(v.alarmCount)],["Alarme mémorisée",fmt(v.alarmLatched)],["Dépassement global",fmt(v.exceedGlobal)]]);}else ['vibrationGlobal','vibrationAxes','vibrationQuality','vibrationCounters'].forEach(id=>dl(id,[["Données",'Jamais reçues']]));
 ...
 if(b4?.hasValue){const v=b4.value;dl('configMeta',[["Fraîcheur PC",`${freshness(b4)} · ${availability(b4)}`],["Version structure",fmt(v.structureVersion)],["Capabilities",hex(v.capabilitiesMask)],["État configuration",fmt(v.configState)],["Erreur configuration",fmt(v.configErrorCode)],["Révision",fmt(v.revisionCounter)],["Config préparée",fmt(v.preparedConfigId)],["CRC préparé",fmt(v.preparedConfigCrc)],["Config active",fmt(v.activeConfigId)],["CRC actif",fmt(v.activeConfigCrc)]]);dl('configPrepared',[["Fréquence échantillonnage",fmt(v.samplingFrequencyHz,' Hz')],["Axes mask",hex(v.axesEnableMask)],["Pleine échelle",fmt(v.fullScaleCode)],["Mode acquisition",fmt(v.acquisitionMode)],["Fenêtre",fmt(v.windowSizeSamples,' échantillons')],["Période indicateur",fmt(v.indicatorPeriodMilliseconds,' ms')],["Durée campagne",fmt(v.campaignDurationSeconds,' s')],["Mode stockage",fmt(v.storageMode)],["Limite stockage",fmt(v.storageLimitMb,' MB')],["RMS warn",fmt(v.rmsWarnThresholdMg,' mg')],["RMS alarm",fmt(v.rmsAlarmThresholdMg,' mg')],["Peak warn",fmt(v.peakWarnThresholdMg,' mg')],["Peak alarm",fmt(v.peakAlarmThresholdMg,' mg')],["Hystérésis",fmt(v.thresholdHysteresisMg,' mg')],["Maintien alarme",fmt(v.alarmHoldTimeMilliseconds,' ms')]]);dl('configActive',[["Fréquence échantillonnage",fmt(v.activeSamplingFrequencyHz,' Hz')],["Axes mask",hex(v.activeAxesEnableMask)],["Pleine échelle",fmt(v.activeFullScaleCode)],["Mode acquisition",fmt(v.activeAcquisitionMode)],["Fenêtre",fmt(v.activeWindowSizeSamples,' échantillons')],["Période indicateur",fmt(v.activeIndicatorPeriodMilliseconds,' ms')],["Durée campagne",fmt(v.activeCampaignDurationSeconds,' s')],["Mode stockage",fmt(v.activeStorageMode)],["Limite stockage",fmt(v.activeStorageLimitMb,' MB')],["RMS warn",fmt(v.activeRmsWarnThresholdMg,' mg')],["RMS alarm",fmt(v.activeRmsAlarmThresholdMg,' mg')],["Peak warn",fmt(v.activePeakWarnThresholdMg,' mg')],["Peak alarm",fmt(v.activePeakAlarmThresholdMg,' mg')],["Hystérésis",fmt(v.activeThresholdHysteresisMg,' mg')],["Maintien alarme",fmt(v.activeAlarmHoldTimeMilliseconds,' ms')]]);dl('configContext',[["Campagne préparée",fmt(v.campaignContextId)],["Mission préparée",fmt(v.missionId)],["Label campagne",fmt(v.campaignLabel)],["Label mission",fmt(v.missionLabel)],["Campagne active",fmt(v.activeCampaignContextId)],["Mission active",fmt(v.activeMissionId)],["Label campagne actif",fmt(v.activeCampaignLabel)],["Label mission actif",fmt(v.activeMissionLabel)]]);}else ['configMeta','configPrepared','configActive','configContext'].forEach(id=>dl(id,[["Données",'Jamais reçues']]));
