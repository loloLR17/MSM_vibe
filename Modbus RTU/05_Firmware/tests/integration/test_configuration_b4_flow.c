...
static void write_prepared_configuration(ConfigurationStagingService *staging,
                                         ...
                                         const ConfigurationPayload *payload)
{
    ...
    uint16_t words[2];
    const uint32_t crc = tr2_b4_prepared_payload_crc(payload);
...
static ModbusBlock4Image project_current(const ConfigurationStagingService *staging,
                                         const ConfigurationWorkflow *workflow)
{
    ...
    source.config_state = (uint16_t)configuration_workflow_state(workflow);
...
int main(void)
{
    ...
    image = project_current(&staging, &workflow);
    assert(image.registers[6] == UINT16_C(CONFIGURATION_STATE_DRAFT));
    ...
    assert(image.registers[6] == UINT16_C(CONFIGURATION_STATE_ACTIVE));
    ...
    assert_write_ok(modbus_write_adapter_write_b4(&staging, UINT16_C(4016), &invalid_frequency, UINT16_C(1)));
    configuration_workflow_note_prepared_payload_modified(&workflow);
    assert(configuration_workflow_state(&workflow) == CONFIGURATION_STATE_DRAFT);
    image = project_current(&staging, &workflow);
    assert(image.registers[6] == UINT16_C(CONFIGURATION_STATE_DRAFT));
    ...
    image = project_current(&staging, &workflow);
    assert(image.registers[6] == UINT16_C(CONFIGURATION_STATE_VALIDATION_ERROR));
    assert(image.registers[16] == UINT16_C(0xFFFF));
    assert(image.registers[100] == UINT16_C(26667));
    assert(image.registers[5] == UINT16_C(42));
    return 0;
