    ...
    CommandJournalEntry *entry)
{
    ...
    if (current.request_identity.command_code != COMMAND_CODE_STOP_ACQUISITION ||
    ...
    if (closed_metadata.campaign_id != campaign_id ||
        closed_metadata.lifecycle_state != CAMPAIGN_LIFECYCLE_CLOSED) {
        return TR2_ERROR_INTERNAL;
    }
    ...
    const CampaignRepository *repository)
{
    ...
    if (entry == NULL || repository == NULL || repository->get_campaign_by_id == NULL ||
        ...
        entry->recovery_context.kind != COMMAND_RECOVERY_CONTEXT_STOP_CAMPAIGN ||
    ...
    if (metadata.campaign_id != entry->recovery_context.value1) {
        return COMMAND_RECONCILIATION_INDETERMINATE;
    }
