...
#define TR2_COMMAND_JOURNAL_BOUNDED_RECORD_MAGIC UINT32_C(0x5452324A)
#define TR2_COMMAND_JOURNAL_BOUNDED_RECORD_ADMISSION_ORDER_OFFSET 62u
#define TR2_COMMAND_JOURNAL_BOUNDED_RECORD_CRC_OFFSET 66u
...
Tr2Result tr2_command_journal_bounded_record_encode(
    const CommandJournalBoundedRecord *record_value,
    uint8_t *record,
    size_t record_size)
{
    ...
    if (record_value == NULL || record == NULL ||
        record_size != TR2_COMMAND_JOURNAL_BOUNDED_RECORD_SIZE ||
        record_value->generation == 0u ||
        record_value->admission_order == 0u ||
    ...
    put_u32_be(&record[0], TR2_COMMAND_JOURNAL_BOUNDED_RECORD_MAGIC);
    put_u16_be(&record[4], TR2_COMMAND_JOURNAL_BOUNDED_RECORD_FORMAT_VERSION);
    put_u16_be(&record[6], (uint16_t)TR2_COMMAND_JOURNAL_BOUNDED_RECORD_SIZE);
    ...
    put_u32_be(&record[TR2_COMMAND_JOURNAL_BOUNDED_RECORD_ADMISSION_ORDER_OFFSET],
               record_value->admission_order);
    crc = crc32_bytes(record, TR2_COMMAND_JOURNAL_BOUNDED_RECORD_CRC_OFFSET);
    put_u32_be(&record[TR2_COMMAND_JOURNAL_BOUNDED_RECORD_CRC_OFFSET], crc);
    return TR2_OK;
}

Tr2Result tr2_command_journal_bounded_record_decode(
    ...
    CommandJournalBoundedRecord *record_value)
{
    ...
    if (get_u32_be(&record[0]) != TR2_COMMAND_JOURNAL_BOUNDED_RECORD_MAGIC ||
        get_u16_be(&record[6]) != (uint16_t)TR2_COMMAND_JOURNAL_BOUNDED_RECORD_SIZE) {
        return TR2_ERROR_CORRUPTED;
    }

    stored_crc = get_u32_be(&record[TR2_COMMAND_JOURNAL_BOUNDED_RECORD_CRC_OFFSET]);
    computed_crc = crc32_bytes(record, TR2_COMMAND_JOURNAL_BOUNDED_RECORD_CRC_OFFSET);
    if (stored_crc != computed_crc) {
        return TR2_ERROR_CORRUPTED;
    }
    if (get_u16_be(&record[4]) != TR2_COMMAND_JOURNAL_BOUNDED_RECORD_FORMAT_VERSION) {
    ...
    record_value->entry.recovery_context.kind =
        (CommandRecoveryContextKind)get_u16_be(&record[30]);
    ...
    record_value->admission_order =
        get_u32_be(&record[TR2_COMMAND_JOURNAL_BOUNDED_RECORD_ADMISSION_ORDER_OFFSET]);
