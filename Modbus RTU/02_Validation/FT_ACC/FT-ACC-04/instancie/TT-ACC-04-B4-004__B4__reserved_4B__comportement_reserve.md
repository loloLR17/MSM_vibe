# TT-ACC-04-B4-004 — Bloc 4 — reserved_4B

## Objectif
Vérifier que le champ réservé `reserved_4B` présente un comportement neutre, stable et sans sémantique cachée.
...
## Scénario / étapes
1. Lire exactement `12` registre(s) à partir de l’adresse `4028` et mémoriser la valeur initiale.
2. Relire la même plage pour vérifier la stabilité read → read à état constant.
3. Tenter une écriture bornée à la plage `4028..4039`.
4. Contrôler le comportement de la tentative : exception Modbus explicite ou acceptation sans modification observable.
5. Relire exactement `12` registre(s) à partir de l’adresse `4028`.
