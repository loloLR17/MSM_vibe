# TT-ACC-04-B0-001 — Bloc 0 — reserved

## Objectif
Vérifier que le champ réservé `reserved` présente un comportement neutre, stable et sans sémantique cachée.
...
## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l’adresse `20` et mémoriser la valeur initiale.
2. Relire la même plage pour vérifier la stabilité read → read à état constant.
3. Tenter une écriture bornée à la plage `20..20`.
4. Contrôler le comportement de la tentative : exception Modbus explicite ou acceptation sans modification observable.
5. Relire exactement `1` registre(s) à partir de l’adresse `20`.
