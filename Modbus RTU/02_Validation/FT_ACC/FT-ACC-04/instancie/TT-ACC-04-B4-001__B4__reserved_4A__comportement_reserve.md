# TT-ACC-04-B4-001 — Bloc 4 — reserved_4A

## Objectif
Vérifier que le champ réservé `reserved_4A` présente un comportement neutre, stable et sans sémantique cachée.
...
## Scénario / étapes
1. Lire exactement `2` registre(s) à partir de l’adresse `4014` et mémoriser la valeur initiale.
2. Relire la même plage pour vérifier la stabilité read → read à état constant.
3. Tenter une écriture bornée à la plage `4014..4015`.
4. Contrôler le comportement de la tentative : exception Modbus explicite ou acceptation sans modification observable.
5. Relire exactement `2` registre(s) à partir de l’adresse `4014`.
