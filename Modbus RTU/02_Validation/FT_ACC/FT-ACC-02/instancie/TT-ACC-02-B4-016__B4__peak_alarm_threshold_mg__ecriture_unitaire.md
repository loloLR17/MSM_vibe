# TT-ACC-02-B4-016 — Bloc 4 — peak_alarm_threshold_mg

## Objectif
Vérifier que le champ logique `peak_alarm_threshold_mg` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4044
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : threshold_hysteresis_mg
- Adresse de début du champ RW suivant attendue : 4045
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4044..4044`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4044` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4044`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4044..4044`.
...
- le champ `peak_alarm_threshold_mg` est modifiable conformément au mapping.
