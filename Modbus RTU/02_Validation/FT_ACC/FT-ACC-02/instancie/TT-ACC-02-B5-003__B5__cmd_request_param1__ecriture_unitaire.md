...
## Objectif
Vérifier que le champ logique `cmd_request_param1` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 5002
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : cmd_request_param2
- Adresse de début du champ RW suivant attendue : 5003
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `5002..5002`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `5002` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `5002`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `5002..5002`.
...
- le champ `cmd_request_param1` est modifiable conformément au mapping.
