...
## Objectif
Vérifier que le champ logique `storage_limit_mb` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4026
- Longueur : 2
- Valeur de test : valeur de test sur 2 registres, différente de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : supervision_enable_mask
- Adresse de début du champ RW suivant attendue : 4040
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4026..4027`.

## Scénario / étapes
1. Lire exactement `2` registre(s) à partir de l'adresse `4026` pour obtenir la valeur initiale.
...
4. Relire exactement `2` registre(s) à partir de l'adresse `4026`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4026..4027`.
...
- le champ `storage_limit_mb` est modifiable conformément au mapping.
