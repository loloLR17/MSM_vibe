# TT-ACC-02-B4-007 — Bloc 4 — window_size_samples

## Objectif
Vérifier que le champ logique `window_size_samples` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : indicator_period_ms
- Adresse de début du champ RW suivant attendue : 4022
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4021..4021`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4021` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4021`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4021..4021`.
...
- le champ `window_size_samples` est modifiable conformément au mapping.
