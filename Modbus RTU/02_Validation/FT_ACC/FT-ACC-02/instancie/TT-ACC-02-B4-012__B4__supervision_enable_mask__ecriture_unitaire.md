# TT-ACC-02-B4-012 — Bloc 4 — supervision_enable_mask

## Objectif
Vérifier que le champ logique `supervision_enable_mask` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : rms_warn_threshold_mg
- Adresse de début du champ RW suivant attendue : 4041
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4040..4040`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4040` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4040`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4040..4040`.
...
- le champ `supervision_enable_mask` est modifiable conformément au mapping.
