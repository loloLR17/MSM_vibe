# TT-ACC-02-B4-005 — Bloc 4 — full_scale_code

## Objectif
Vérifier que le champ logique `full_scale_code` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4019
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : acquisition_mode
- Adresse de début du champ RW suivant attendue : 4020
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4019..4019`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4019` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4019`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4019..4019`.
...
- le champ `full_scale_code` est modifiable conformément au mapping.
