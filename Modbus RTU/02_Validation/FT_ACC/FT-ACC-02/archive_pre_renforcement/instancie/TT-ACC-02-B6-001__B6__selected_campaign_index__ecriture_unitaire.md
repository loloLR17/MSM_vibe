...
## Objectif
Vérifier que le champ logique `selected_campaign_index` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `6003..6003`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `6003` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `6003`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `6003..6003`.
...
- le champ `selected_campaign_index` est modifiable conformément au mapping.
