...
## Objectif
Vérifier que le champ logique `cmd_request_code` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : cmd_request_transaction_id
- Adresse de début du champ RW suivant attendue : 5001
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `5000..5000`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `5000` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `5000`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `5000..5000`.
...
- le champ `cmd_request_code` est modifiable conformément au mapping.
