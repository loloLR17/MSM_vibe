# TT-ACC-02-B4-008 — Bloc 4 — indicator_period_ms

## Objectif
Vérifier que le champ logique `indicator_period_ms` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4022
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : campaign_duration_s
- Adresse de début du champ RW suivant attendue : 4023
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4022..4022`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4022` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4022`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4022..4022`.
...
- le champ `indicator_period_ms` est modifiable conformément au mapping.
