# TT-ACC-02-B4-017 — Bloc 4 — threshold_hysteresis_mg

## Objectif
Vérifier que le champ logique `threshold_hysteresis_mg` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4045
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : alarm_hold_time_ms
- Adresse de début du champ RW suivant attendue : 4046
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4045..4045`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4045` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4045`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4045..4045`.
...
- le champ `threshold_hysteresis_mg` est modifiable conformément au mapping.
