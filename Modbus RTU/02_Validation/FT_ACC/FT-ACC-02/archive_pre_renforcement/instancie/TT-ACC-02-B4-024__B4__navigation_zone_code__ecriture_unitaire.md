# TT-ACC-02-B4-024 — Bloc 4 — navigation_zone_code

## Objectif
Vérifier que le champ logique `navigation_zone_code` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4093
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : load_state_code
- Adresse de début du champ RW suivant attendue : 4094
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4093..4093`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4093` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4093`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4093..4093`.
...
- le champ `navigation_zone_code` est modifiable conformément au mapping.
