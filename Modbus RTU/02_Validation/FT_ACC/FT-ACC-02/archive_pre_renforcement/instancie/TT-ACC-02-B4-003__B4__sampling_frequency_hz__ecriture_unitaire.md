...
## Objectif
Vérifier que le champ logique `sampling_frequency_hz` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : axes_enable_mask
- Adresse de début du champ RW suivant attendue : 4018
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4016..4016`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4016` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4016`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4016..4016`.
...
- le champ `sampling_frequency_hz` est modifiable conformément au mapping.
