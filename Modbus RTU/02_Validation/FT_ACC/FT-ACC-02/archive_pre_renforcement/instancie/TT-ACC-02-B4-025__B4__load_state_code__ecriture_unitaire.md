# TT-ACC-02-B4-025 — Bloc 4 — load_state_code

## Objectif
Vérifier que le champ logique `load_state_code` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4094
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : sea_state_code
- Adresse de début du champ RW suivant attendue : 4095
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4094..4094`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4094` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4094`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4094..4094`.
...
- le champ `load_state_code` est modifiable conformément au mapping.
