# TT-ACC-02-B4-018 — Bloc 4 — alarm_hold_time_ms

## Objectif
Vérifier que le champ logique `alarm_hold_time_ms` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4046
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : campaign_context_id
- Adresse de début du champ RW suivant attendue : 4056
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4046..4046`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4046` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4046`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4046..4046`.
...
- le champ `alarm_hold_time_ms` est modifiable conformément au mapping.
