# TT-ACC-02-B4-020 — Bloc 4 — mission_id

## Objectif
Vérifier que le champ logique `mission_id` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : campaign_label
- Adresse de début du champ RW suivant attendue : 4060
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4058..4059`.

## Scénario / étapes
1. Lire exactement `2` registre(s) à partir de l'adresse `4058` pour obtenir la valeur initiale.
...
4. Relire exactement `2` registre(s) à partir de l'adresse `4058`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4058..4059`.
...
- le champ `mission_id` est modifiable conformément au mapping.
