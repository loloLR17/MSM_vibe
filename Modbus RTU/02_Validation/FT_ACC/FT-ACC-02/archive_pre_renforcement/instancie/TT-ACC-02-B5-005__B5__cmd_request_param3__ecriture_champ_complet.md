...
## Objectif
Vérifier que le champ logique `cmd_request_param3` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
- Champ RW suivant attendu : cmd_request_confirm_key
- Adresse de début du champ RW suivant attendue : 5006
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `5004..5005`.

## Scénario / étapes
1. Lire exactement `2` registre(s) à partir de l'adresse `5004` pour obtenir la valeur initiale.
...
4. Relire exactement `2` registre(s) à partir de l'adresse `5004`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `5004..5005`.
...
- le champ `cmd_request_param3` est modifiable conformément au mapping.
