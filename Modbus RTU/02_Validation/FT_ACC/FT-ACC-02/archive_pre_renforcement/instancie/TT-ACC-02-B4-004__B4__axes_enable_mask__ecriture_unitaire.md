...
## Objectif
Vérifier que le champ logique `axes_enable_mask` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 4018
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : full_scale_code
- Adresse de début du champ RW suivant attendue : 4019
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `4018..4018`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `4018` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `4018`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `4018..4018`.
...
- le champ `axes_enable_mask` est modifiable conformément au mapping.
