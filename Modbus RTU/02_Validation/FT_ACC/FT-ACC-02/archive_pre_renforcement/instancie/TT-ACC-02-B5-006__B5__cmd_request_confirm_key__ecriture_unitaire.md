...
## Objectif
Vérifier que le champ logique `cmd_request_confirm_key` déclaré `RW` accepte une écriture conforme et restitue la valeur écrite en lecture immédiate.
...
## Données d’entrée
- Adresse de départ : 5006
- Longueur : 1
- Valeur de test : mot de test différent de la valeur initiale

## Contrôle de frontière
- Champ RW suivant attendu : cmd_request_control
- Adresse de début du champ RW suivant attendue : 5007
- Vérifier l’absence de débordement de l’écriture au-delà de la plage `5006..5006`.

## Scénario / étapes
1. Lire exactement `1` registre(s) à partir de l'adresse `5006` pour obtenir la valeur initiale.
...
4. Relire exactement `1` registre(s) à partir de l'adresse `5006`.
5. Vérifier que la valeur relue correspond à la valeur écrite.
6. Vérifier que l’opération reste bornée à la plage `5006..5006`.
...
- le champ `cmd_request_confirm_key` est modifiable conformément au mapping.
