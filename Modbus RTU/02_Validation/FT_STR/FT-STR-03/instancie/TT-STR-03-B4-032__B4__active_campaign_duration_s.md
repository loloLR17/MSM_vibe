# TT-STR-03-B4-032 — Bloc 4 — active_campaign_duration_s

## Objectif
Valider l'encodage uint32 (MSW puis LSW) du champ `active_campaign_duration_s`.

## Référence mapping
- Adresse début : 4106
- Adresse fin : 4107
- Type : uint32

## Préconditions
- FT-STR-02 validée
- Accès Modbus opérationnel

## Étapes
1. Lire les registres 4106 et 4107.
2. Reconstituer la valeur en MSW/LSW.
3. Reconstituer la valeur en LSW/MSW (inverse).
4. Comparer les deux interprétations.

## Résultat attendu
- seule l'interprétation MSW/LSW est cohérente.

## Critères d’acceptation
- cohérence MSW/LSW validée
- incohérence LSW/MSW
