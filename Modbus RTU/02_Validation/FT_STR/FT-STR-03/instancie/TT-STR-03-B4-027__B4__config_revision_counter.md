# TT-STR-03-B4-027 — Bloc 4 — config_revision_counter

## Objectif
Valider l'encodage uint32 (MSW puis LSW) du champ `config_revision_counter`.

## Référence mapping
- Adresse début : 4012
- Adresse fin : 4013
- Type : uint32

## Préconditions
- FT-STR-02 validée
- Accès Modbus opérationnel

## Étapes
1. Lire les registres 4012 et 4013.
2. Reconstituer la valeur en MSW/LSW.
3. Reconstituer la valeur en LSW/MSW (inverse).
4. Comparer les deux interprétations.

## Résultat attendu
- seule l'interprétation MSW/LSW est cohérente.

## Critères d’acceptation
- cohérence MSW/LSW validée
- incohérence LSW/MSW
