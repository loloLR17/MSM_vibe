# TT-STR-02-B3-015 — Bloc 3 — B3_PEAK_X_MG

## Objectif
Valider que le champ logique `B3_PEAK_X_MG` est exposé au bon emplacement, avec le bon type et la bonne taille, conformément au mapping unifié.
...
- Vérifier l'absence d'empiètement entre `B3_PEAK_X_MG` et `B3_PEAK_Y_MG`.

## Étapes
1. Lire exactement `2` registre(s) à partir de l'adresse `3024`.
2. Vérifier que la lecture couvre la plage `3024` à `3025` sans décalage.
3. Vérifier que la taille observée correspond exactement à `2` registre(s).
4. Vérifier que le champ logique suivant commence à l'adresse `3026`.
5. Vérifier que l'interprétation du champ reste compatible avec le type `uint32`.

## Résultat attendu
- la lecture de `B3_PEAK_X_MG` est possible sur la plage `3024` à `3025` ;
